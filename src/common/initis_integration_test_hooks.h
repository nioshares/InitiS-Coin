...
#ifndef INITIS_INTEGRATION_TEST_HOOKS_H
#define INITIS_INTEGRATION_TEST_HOOKS_H
...
#endif // INITIS_INTEGRATION_TEST_HOOKS_H

//
// CPP Implementation
//
#ifdef INITIS_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
#include <string.h>
#include <assert.h>
#include <chrono>
#include <thread>
...
, "initi-default-integration-test-mem-name"
, false
};
...
void initi::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void initi::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void initi::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("InitiS Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "InitiS Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "InitiS Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "InitiS Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "InitiS Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("InitiS Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void initi::deinit_integration_test_context()
{
...
std::vector<std::string> initi::separate_stdin_to_space_delim_args(initi::fixed_buffer const *cmd)
{
...
initi::fixed_buffer initi::read_from_stdin_shared_mem()
{
...
void initi::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
...
#endif // INITIS_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
