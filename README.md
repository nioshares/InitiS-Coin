...
`$ cd initi && git submodule init && git submodule update`

### Build instructions

InitiS uses the CMake build system and a top-level [Makefile](Makefile) that
...
* Change to the root of the source code directory, change to the most recent release branch, and build:
    ...
    *Note*: The instructions above will compile the most stable release of the
    InitiS software. If you would like to use and test the most recent software,
...
* Add `PATH="$PATH:$HOME/initi/build/release/bin"` to `.profile`
...
* Install the dependencies for InitiS from the 'Debian' column in the table above.
...
* Clone initi and checkout most recent release version:
```
        git clone https://github.com/InitiS-Coin/initi.git
	cd initi
	git checkout master
```
* Build:
...
If you are using the older Raspbian Jessie image, compiling InitiS is a bit more complicated. The version of Boost available in the Debian Jessie repositories is too old to use with InitiS, and thus you must compile a newer version yourself. The following explains the extra steps, and has been tested on a Raspberry Pi 2 with a clean install of minimal Raspbian Jessie.
...
* Then, install the dependencies for InitiS except `libunwind` and `libboost-all-dev`
...
* From here, follow the [general Raspberry Pi instructions](#on-the-raspberry-pi) from the "Clone initi and checkout most recent release version" step.
...
* The resulting executables can be found in `build/<MinGW version>/<initi version>/release/bin`
...
* The resulting executables can be found in `build/<MinGW version>/<initi version>/debug/bin`

### On FreeBSD:

The project can be built from scratch by following instructions for Linux above. If you are running initi in a jail you need to add the flag: `allow.sysvipc=1` to your jail configuration, otherwise lmdb will throw the error message: `Failed to open lmdb environment: Function not implemented`.
...
You will have to add the serialization, date_time, and regex modules to Boost when building as they are needed by InitiS.
...
Build initi: `env DEVELOPER_LOCAL_TOOLS=1 BOOST_ROOT=/usr/local make release-static`
...
### On Linux for Android (using docker):
        ...
        docker build -f utils/build_scripts/android32.Dockerfile -t initi-android .
        # Build image (for ARM 64-bit)
        docker build -f utils/build_scripts/android64.Dockerfile -t initi-android .
        # Create container
        docker create -it --name initi-android initi-android bash
        # Get binaries
        docker cp initi-android:/src/build/release/bin .
...
Using `depends` might also be easier to compile InitiS on Windows than using MSYS. Activate Windows Subsystem for Linux (WSL) with a distro (for example Ubuntu), install the apt build-essentials and follow the `depends` steps as depicted above.

The produced binaries still link libc dynamically. If the binary is compiled on a current distribution, it might not run on an older distribution with an older installation of libc. Passing `-DBACKCOMPAT=ON` to cmake will make sure that the binary will run on systems having at least libc version 2.17.

## Installing InitiS from a package

* Docker

        # Build using all available cores
        docker build -t initi .

        # or build using a specific number of cores (reduce RAM requirement)
        docker build --build-arg NPROC=1 -t initi .

        # either run in foreground
        docker run -it -v /initi/chain:/root/.initi -v /initi/wallet:/wallet -p 24542:24542 initi

        # or in background
        docker run -it -d -v /initi/chain:/root/.initi -v /initi/wallet:/wallet -p 24542:24542 initi
...
While InitiS isn't made to integrate with Tor, it can be used wrapped with torsocks, by
...
This section contains general instructions for debugging failed installs or problems encountered with InitiS. First ensure you are running the latest version built from the Github repo.
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as initisd. It may be named just `core`, or `core.xxxx` with numbers appended.
...
Configure InitiS with the -D SANITIZE=ON cmake flag, eg:

    cd build/debug && cmake -D SANITIZE=ON -D CMAKE_BUILD_TYPE=Debug ../..

You can then run the initi tools normally. Performance will typically halve.
...
`cd ~/initi/external/db_drivers/liblmdb && make`
