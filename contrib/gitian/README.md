...
*Setup instructions for a Gitian build of InitiS using a VM or physical system.*

Gitian is the deterministic build process that is used to build the InitiS CLI
...
```bash
cp initi/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```
./gitian-build.py --setup initi-user x.x.x
```

Where `initi-user` is your Github name and `x.x.x` is the version tag you want to build.
...
```
git clone git@github.com:InitiS-Coin/gitian.sigs.git
git remote add initi-user git@github.com:initi-user/gitian.sigs.git
```
...
To build the most recent tag:
 ...
 `./gitian-build.py --detach-sign --no-commit -b initi-user 2.0.0`
...
```
export NAME=initi-user
export VERSION=2.0.0
gpg --output $VERSION-linux/$NAME/initi-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/initi-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/initi-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/initi-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/initi-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/initi-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[InitiS-Coin/gitian.sigs](https://github.com/InitiS-Coin/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/initi-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/initi-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/initi-osx-*-build.assert
```
