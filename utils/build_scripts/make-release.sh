...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    mv build/release/bin initi-$build-x64-$version
    zip -r initi-$build-x64-$version.zip initi-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum initi-*-x64-$version.zip
